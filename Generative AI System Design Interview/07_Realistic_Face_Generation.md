...
<p align="center">Figure 1: Realistic faces generated by StyleGAN2 [1]</p>
...
<p align="center">Figure 2: Input and output of a face generation system</p>
...
<p align="center">Figure 3: VAE training and inference process</p>
...
<p align="center">Figure 4: GAN training and inference process</p>
...
<p align="center">Figure 5: Autoregressive model training and inference process</p>
...
- **Support of multimodal conditioning:** Autoregressive models easily support conditioning on different modalities. For example, if we provide a celebration audio as input, the generated image will match the audio.
...
<p align="center">Figure 6: Diffusion model training and inference process</p>
...
- **Robustness to noisy images:** Diffusion models are effective at removing noise from images because of their denoising process.
...
<p align="center">Figure 7: Facial attribute manipulation. Images are taken from [10].</p>
...
<p align="center">Figure 8: Series of upsampling blocks</p>
...
<p align="center">Figure 9: Transpose conv with 3x3 filter and stride=1 over 4x4 inputs [11]</p>
...
<p align="center">Figure 10: Comparison of different normalization methods [15]</p>
...
<p align="center">Figure 11: Generator architecture</p>
...
<p align="center">Figure 12: Series of downsampling blocks</p>
...
<p align="center">Figure 13: Discriminator architecture</p>
...
<p align="center">Figure 14: Generator and discriminator alternative training</p>
...
$$L_D = -\frac{1}{m} \sum_{i=1}^{m} \log D(x^{(i)}) - \frac{1}{n} \sum_{j=1}^{n} \log(1 - D(G(z^{(j)})))$$

Where:

- D(x^(i)) is the discriminator's predicted probabilities for a real image,
...
<p align="center">Figure 15: Latent space points map to face images</p>
...
<p align="center">Figure 16: Random vs. truncated sampling (gray regions represent sampling areas)</p>
...
2. **Computing class probabilities:** For each generated image, the Inception model provides a probability distribution over all of its 1,000 object classes. A high-quality image should lead to a distribution with a peak indicating that the model recognizes it as a clear instance of a class.
3. **Calculating the marginal distribution:** Marginal distribution refers to the average of the predicted class probabilities across all images. If the images are diverse, the marginal distribution will be flat and spread across many classes.
4. **Computing KL divergence:** The KL divergence measures how different the predicted class distribution for each image is from the marginal distribution. High-quality images will have a distribution that is very different from the marginal distribution.
...
<p align="center">Figure 17: Inception score calculation</p>
...
- **Quality:** High-quality images result in a sharp, peaked probability distribution, indicating that the image is clearly recognized as belonging to a particular class.

#### Fréchet inception distance (FID)

FID is another popular metric for evaluating the quality of images produced by generative models. It assesses how similar the distribution of generated images is to the distribution of real images. Unlike the Inception score, which uses class probabilities, FID considers the statistics of the features extracted by a pretrained model such as Inception v3.

Here's a step-by-step explanation of how FID is calculated:

1. **Generating images:** We start by generating a large set of images using the model we want to evaluate.
2. **Extracting features:** We pass each image (both generated and real) through the Inception v3 model and extract features ("activations") from a specific layer, usually one near the end of the network.
3. **Calculating mean and covariance:** We calculate the mean and covariance of the extracted features separately for generated and real images.
4. **Computing Fréchet distance:** We calculate the FID as the Fréchet distance between the mean and covariance of generated and real images. A lower FID indicates greater similarity between the distributions, meaning the generated images are more realistic and diverse. To learn more about the Fréchet distance and its formula, refer to [29].

![alt text](image-203.png)

<p align="center">Figure 18: FID calculation</p>

How does the FID measure both diversity and quality?

- **Diversity:** The FID considers the covariance of the features, reflecting the spread and variation in the image features.
- **Quality:** FID ensures the generated images are high-quality by comparing their feature distributions to those of real images.
...
<p align="center">Figure 19: Pairwise comparison during human evaluation</p>
...
- **Latency:** Latency refers to the time it takes from when a request is made until the image is fully generated and delivered to the user. Fast response times are crucial for maintaining a good user experience, especially in interactive applications.
...
<p align="center">Figure 20: Realistic face generation overall design</p>
