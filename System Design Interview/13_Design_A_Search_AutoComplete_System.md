...
- Assume 20% of the daily queries are new. 10 million × 10 queries / day × 20 byte per query × 20% = 0.4 GB. This means 0.4 GB of new data is added to storage daily.
...
The time complexity of this algorithm is the sum of time spent on each step mentioned above: O(p) + O(c) + O(c log c)
...
**Aggregated Data.** Table 4 shows an example of aggregated weekly data. "time" field represents the start time of a week. "frequency" field is the sum of the occurrences for the corresponding query in that week.
